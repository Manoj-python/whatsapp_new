...
def get_contacts_page3(page=1, size=100, q=""):
    ...
    for mobile in mobile_list:
        ...
        latest_msg = SmsWhatsAppLog3.objects.filter(
            mobile=mobile
        ).order_by('-sent_at').first()

        if latest_msg:
            # Count unread messages
            unread = SmsWhatsAppLog3.objects.filter(
                mobile=mobile,
...
@sync_to_async
def get_messages_page_from_db3(mobile, page=1, size=30):
    """
    ...
    total = SmsWhatsAppLog3.objects.filter(mobile=mobile_norm).count()
...
@sync_to_async
def create_outgoing_log3(mobile: str, text: str, message_id: str, content_type: str="text", media_filename: Optional[str]=None):
    """
...
def mark_messages_read_db3(mobile: str):
    ...
    """
    SmsWhatsAppLog3.objects.filter(
        mobile=format_mobile(mobile),
        message_type="Received",
        status="Unread"
    ).update(status="Read")
...
# -------------------------
def send_text_via_whatsapp3(to_number: str, text_body: str) -> dict:
    """Send text message via WhatsApp API"""
    url = f"https://graph.facebook.com/v17.0/{settings.WHATSAPP3_PHONE_NUMBER_ID}/messages"
    headers = {
...
class ChatConsumer3(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            await self._add_to_group("contacts_group3")
            await self._add_to_group("presence_group3")
    ...
    async def _handle_get_contacts(self, content):
        ...
        try:
            ...
            res = await get_contacts_page3(page=page, q=q)
    ...
    async def _handle_get_messages(self, content):
        ...
        try:
            ...
            res = await get_messages_page_from_db3(mobile, page, size)
    ...
    async def _handle_send_message(self, content):
        ...
        try:
            ...
            created = await create_outgoing_log3(mobile, text, "", content_type)
    ...
    async def _send_to_whatsapp_background(self, mobile, text, content_type, log_id, agent_name):
        ...
        try:
            if content_type == "text":
                send_resp = await sync_to_async(send_text_via_whatsapp3)(mobile, text)
            else:
            ...
            if msg_id:
                await sync_to_async(
                    lambda: SmsWhatsAppLog3.objects.filter(id=log_id).update(message_id=msg_id)
                )()
