...
def get_cached_unread_count():
    """Cache unread count globally to avoid full table scans"""
    count = cache.get("global_unread_count_msg")
    if count is None:
        count = ChatContact.objects.filter(unread__gt=0).exclude(
            Q(last_msg__icontains="No messages yet") | Q(last_msg="")
        ).count()
        cache.set("global_unread_count_msg", count, timeout=3)
    return count
...
def get_contacts_page(page=1, size=30, q="", filter_type="all", level=None, group_ids=None):
    ...
    SELECT 
        ...
        c.current_level,
        NULL AS group_name
    FROM messaging_chatcontact c
    LEFT JOIN (
        SELECT l.mobile, l.sent_text_message
        FROM messaging_smswhatsapplog l
        JOIN (
...
def create_outgoing_log(mobile: str, text: str, message_id: str, content_type: str = "text",
    ...
    cache.delete("global_unread_count_msg")
    return {
        "id": log.id,
        "mobile": log.mobile,
...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def _handle_get_contacts(self, content):
        try:
            ...
            # Cached contacts query
            cache_key = contacts_cache_key(page, size, q, filter_type, level, str(group_ids))
            res = cache.get(cache_key)
            if res is None:
                res = await get_contacts_page(page, size, q, filter_type, level, group_ids)
                cache.set(cache_key, res, timeout=5)   # 5 seconds cache
    ...
    async def _handle_get_messages(self, content):
        try:
            mobile = content.get("mobile")
            before_date = content.get("before_date")
            ...
            if self.connection_active:
                await self.send_json({
                    ...
                    "is_initial": before_date is None and before_id is None
                })
        except Exception as e:
    ...
    async def _handle_send_message(self, content):
        try:
            ...
            # Do NOT call cache.clear() - it destroys all caches
            # Instead, we rely on short TTL and invalidate unread count separately
            cache.delete("global_unread_count_msg")
